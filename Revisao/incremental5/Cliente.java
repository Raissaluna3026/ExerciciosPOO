...
class Cliente {
    ...
    ublic CartaoDeCredito getCartao(String numeroCartao) {
    ...
    public void adicionarCartao(CartaoDeCredito cartao) {
        if (!cartoesDeCredito.contains(cartao)) { // Verifica se o cartão já foi adicionado
            this.cartoesDeCredito.add(cartao);
            System.out.println("Cartão adicionado ao cliente " + nome);
        } else {
            System.out.println("Esse cartão já está associado ao cliente.");
        }
    }
